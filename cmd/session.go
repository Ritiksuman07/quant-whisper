...
func bindRuntimeFlags(flags cmdFlagBinder, values *runtimeFlags) {
	...
	flags.StringVar(&values.logFormat, "log-format", values.logFormat, "logger format (text|json)")
}
...
func runSession(ctx context.Context, mode quantwhisperer.Mode, options config.Options, out io.Writer, logFormat string) error {
	logger := newLogger(logFormat)
	brokerClient, marketData, err := buildSessionDependencies(mode, options, logger)
	if err != nil {
		return err
	}

	session, err := engine.NewSession(options, brokerClient, marketData, logger)
	if err != nil {
		return err
	}
	defer session.Close()
...
func buildSessionDependencies(mode quantwhisperer.Mode, options config.Options, logger *slog.Logger) (broker.Client, engine.MarketDataProvider, error) {
	client, err := broker.NewClient(options.Broker)
	if err != nil {
		return nil, nil, err
	}
	componentLogger := logger.With(slog.String("component", "market_data"))

	switch mode {
	case quantwhisperer.ModePaper:
		return client, engine.NewSyntheticProvider(options.Broker, options.TickInterval, options.MaxTicks, componentLogger), nil
	case quantwhisperer.ModeLive:
		return client, engine.NewLiveProvider(client, options.TickInterval, options.MaxTicks, componentLogger), nil
	default:
		return nil, nil, fmt.Errorf("unsupported mode for market data provider: %s", mode)
	}
}
